...
do
 ...
 local function getCellRowCol(cellPos)
 		local row,col=Utility.cell_to_rowcol(cellPos);
 		row=row+1;
 		col=col+1;
 		return row,col
 end
...
local function load_sheet(self, result)
	...
	local maxCol =1;
	for _,v in ipairs(result.sheetData) do
		local row,col=getCellRowCol(v.r);
		if not rows[row] then
			rows[row]={};
		end
		local colTbl=rows[row];
	...
	result.sheetRaw=rows;

	--最大格子数

	-- local rangeStr=result.dimension ;
	-- local startRange,endRange=rangeStr:match("([^:]+):([^:]+)")
	-- --只有一个格子的情况
	-- if endRange == nil then
	-- 	--print("startRange:"..tostring(startRange).."|endRange:"..tostring(endRange))
	-- 	endRange=rangeStr;
	-- end
	-- local maxRow,maxCol=Utility.cell_to_rowcol(endRange);
	local dimensionTbl={["rowCount"]=maxRow,["colCount"]= maxCol};
	result.sheetSize=dimensionTbl;
